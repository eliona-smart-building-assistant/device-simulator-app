...
var ConfigurationWhere = struct {
	...
	ID:                whereHelperint64{field: "\"device_simulator\".\"configuration\".\"id\""},
	APIAccessChangeMe: whereHelperstring{field: "\"device_simulator\".\"configuration\".\"api_access_change_me\""},
	RefreshInterval:   whereHelperint32{field: "\"device_simulator\".\"configuration\".\"refresh_interval\""},
	RequestTimeout:    whereHelperint32{field: "\"device_simulator\".\"configuration\".\"request_timeout\""},
	AssetFilter:       whereHelpertypes_JSON{field: "\"device_simulator\".\"configuration\".\"asset_filter\""},
	Active:            whereHelperbool{field: "\"device_simulator\".\"configuration\".\"active\""},
	Enable:            whereHelperbool{field: "\"device_simulator\".\"configuration\".\"enable\""},
	ProjectIds:        whereHelpertypes_StringArray{field: "\"device_simulator\".\"configuration\".\"project_ids\""},
	UserID:            whereHelperstring{field: "\"device_simulator\".\"configuration\".\"user_id\""},
}
...
func (o *Configuration) Assets(mods ...qm.QueryMod) assetQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"device_simulator\".\"asset\".\"configuration_id\"=?", o.ID),
	)
...
func (configurationL) LoadAssets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeConfiguration interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`device_simulator.asset`),
		qm.WhereIn(`device_simulator.asset.configuration_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, rel := range related {
		if insert {
			...
			updateQuery := fmt.Sprintf(
				"UPDATE \"device_simulator\".\"asset\" SET %s WHERE %s",
...
func Configurations(mods ...qm.QueryMod) configurationQuery {
	mods = append(mods, qm.From("\"device_simulator\".\"configuration\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"device_simulator\".\"configuration\".*"})
	}
...
func FindConfiguration(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*Configuration, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"device_simulator\".\"configuration\" where \"id\"=$1", sel,
	)
...
func (o *Configuration) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"device_simulator\".\"configuration\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"device_simulator\".\"configuration\" %sDEFAULT VALUES%s"
		}
...
func (o *Configuration) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"device_simulator\".\"configuration\" SET %s WHERE %s",
...
func (o ConfigurationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"device_simulator\".\"configuration\" SET %s WHERE %s",
...
func (o *Configuration) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"device_simulator\".\"configuration\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *Configuration) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"device_simulator\".\"configuration\" WHERE \"id\"=$1"
...
func (o ConfigurationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"device_simulator\".\"configuration\" WHERE " +
...
func (o *ConfigurationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"device_simulator\".\"configuration\".* FROM \"device_simulator\".\"configuration\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, configurationPrimaryKeyColumns, len(*o))
...
func ConfigurationExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"device_simulator\".\"configuration\" where \"id\"=$1 limit 1)"
